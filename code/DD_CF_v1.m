...
function [DBSig] = DD_CF_v1(Sig)
    ...
    %%%Location of high-amplitude peaks/高幅值谱峰的定位%%%
    xdata = tmax;
    ydata = SigButtermax;
    ...
    %%%Truncate each peak, find the length of each segment, and judge
    ...
    if imfbegin == imfend   %Determine whether the number of IMFs dominated by the baseline is 1
        DBSig = SigButter - modes(imfend,length(Lsection)+1:length(Lsection)+length(SigButter));
    else
        DBSig = SigButter - sum((modes(imfbegin:imfend,length(Lsection)+1:length(Lsection)+length(SigButter))));    
    end
